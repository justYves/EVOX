app.factory('CameraFactory', function($rootScope, PointerFactory) {
  ...
  function startCamera(game) {
    ...
    function onDocumentKeyUp(event) {
      switch (event.keyCode) {
        case 16:
          ...
          console.log("shift is:", isShiftDown)
          break;
      }
    }
    ...
    function onDocumentKeyDown(event) {
      switch (event.keyCode) {
        case 39:
          if (isShiftDown) rotateCamera();
          break;
        case 37:
          if (isShiftDown) rotateCamera(true);
          break;
      }
    }


    function rotateCamera(inverse) {
      // console.log(radius);
      var increment = inverse ? -0.5 : 0.5;
      //controls are set in the stream in the voxel engine/ voxel control / stream on event(attain)
      // theta = -(1* 0.5) + onMouseDownTheta
      // phi = (1 * 0.5) + onMouseDownPhi
      theta += increment;
